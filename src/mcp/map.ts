...
const mapUri = 'ui://santander/bus-stops-map-v2.html';
...
<body>
    ...
    <script>
        ...
        function popupContent(stop) {
            ...
            details.textContent = [stop.id && 'Stop ' + stop.id, stop.address].filter(Boolean).join(' · ');
        ...
        function render(data) {
            ...
            title.textContent = data.title || 'Santander bus stops';
            markers.clearLayers();
            const bounds = [];
            for (const stop of data.stops) {
                const point = [stop.latitude, stop.longitude];
                L.marker(point).bindPopup(popupContent(stop)).addTo(markers);
                bounds.push(point);
            }
            empty.hidden = true;
...
export function registerBusMap(server: McpServer, widgetDomain: string) {
    registerAppResource(
        server,
        'Santander bus stops map',
        mapUri,
        { description: 'Interactive map used by the Santander bus-stop map renderer.' },
        async () => ({
            contents: [{
                uri: mapUri,
